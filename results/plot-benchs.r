...
no_filter_f <- function(df_input) {
	return(list(df_input, c(""), "result", baseline_mapfunc, "PLL | 1.10V | 150MHz"))
}
...
pll_rosc <- function(df_input) {
	return(list(df_input, c(""), "pll_rosc", baseline_mapfunc, ""))
}
...
for(expe in c(no_filter_f)) {
	...
	#names(myColors) <- levels(df_expe$benchmark_name)
	#df_expe <- df_expe %>% filter(current_timestamp <= 10)
	mtimestamp <- max(df_expe$current_timestamp, na.rm = TRUE)
	p1 <- ggplot(df_expe, aes(x = current_timestamp, y = power_sample, color=interaction(benchmark_name, iteration_num), group=interaction(benchmark_name, iteration_num))) + 
		...
		#scale_colour_manual(name = "Configuration:", values = myColors) +
	...
	energy_consumption <- df_expe %>%
		...
		reframe(benchmark_name = benchmark_name, clock_source = clock_source, vreg_output = vreg_output, clock_freq = clock_freq, gp = gp, avg_energy = mean(energy_sample), std_energy = sd(energy_sample), avg_time = mean(current_timestamp), std_time = sd(current_timestamp), .groups = "drop") %>%
		distinct()
	
	p2 <- ggplot(energy_consumption , aes(x = benchmark_name, y = avg_energy, fill=benchmark_name)) +
		...
		#scale_fill_manual(name = "Configuration:", values = myColors) +
	...
	#		energy_lora_rel = round(energy_lora, 2), 
	#		#total_energy = energy_mat_mul_double
	#		total_energy = round(energy_lora, 2)
	#	)
	#	
	#energy_consumption_table <- energy_consumption_table %>%
	#	mutate(

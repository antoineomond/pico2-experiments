...
df <- df %>% filter(iteration_num == 0) %>% filter(expe_num < 4)
...
p <- ggplot(df, aes(x = current_timestamp, y = current_sample, color=factor(expe_num), group=factor(expe_num))) +
	...
	geom_hline(yintercept = median(df[df$expe_num == 0,]$current_sample, na.rm = TRUE), color = "black") +
	geom_hline(yintercept = median(df[df$expe_num == 1,]$current_sample, na.rm = TRUE), color = "purple") +
	geom_hline(yintercept = median(df[df$expe_num == 2,]$current_sample, na.rm = TRUE), color = "dark grey") +
	geom_hline(yintercept = median(df[df$expe_num == 3,]$current_sample, na.rm = TRUE), color = "dark green") +

...
baseline_mapfunc <- function(lvls) { return(gsub("\\..+", "", lvls)) }
...
#for(expe in c(pll_f)) {
	...
	#df_expe <- df_expe %>% filter(iteration_num == 1)
	#df_expe <- baseline_f(df_expe)
	...
	df_expe$gp <- interaction(df_expe$clock_source, df_expe$vreg_output, paste(round(df_expe$clock_freq/1000000, 1), "MHz", sep=""))
